...
src/app/api/household/members/route.ts         GET: household info + member list with user data (includes email); resolves the household via getUserHousehold (honours users.active_household_id, falls back to most-recent) so the household switcher changes what this returns
...
src/components/admin/HideTestToggle.tsx        Shared dark-theme checkbox for the hide-test toggle (used on Overview/Users/Households).
src/components/admin/InfoField.tsx             Shared labelled value for admin expandable rows; mono fields (IDs) get a copy-to-clipboard button. Replaces the per-page InfoField in users + households pages.
...
src/app/api/admin/stats/route.ts               GET: 5 parallel sql queries — totalUsers (app users table), totalHouseholds, premium/free counts, signupsOverTime, conversionsOverTime (90 days). Honors ?hideTest=true (injects EXCLUDE_TEST_* fragments).
src/app/api/admin/users/route.ts               GET: paginated user list with search + filter + ?hideTest, camelCase response
src/app/api/admin/users/[id]/route.ts          DELETE: remove a user. ?deleteHousehold=true wipes their household first (single DELETE FROM households; all household FKs are onDelete cascade). Otherwise ANONYMIZES the account (revokes sessions/accounts, drops memberships, scrubs PII + sets users.deleted_at, tombstones better-auth user email) rather than hard-deleting the users row — because ~20 content tables reference users.id with no onDelete, so a hard delete would FK-violate. Preserves surviving-household history as "Former Member".
src/app/api/admin/households/route.ts          GET: paginated household list with ARRAY_AGG member emails, search by name OR invite code, filter, ?hideTest, camelCase response
...
src/app/(admin)/layout.tsx                     Admin shell passthrough. Auth is enforced by the API routes (requireAdminSession); pages redirect to /admin/login on 401. Nav + dark theme live in src/app/(admin)/admin/layout.tsx (client).
src/app/(admin)/admin/login/page.tsx           Admin login: dark card, indigo (#6366F1) branding, POST to /api/admin/login
src/app/(admin)/admin/page.tsx                 Admin overview: 4 StatCards (Users/Households/Premium/Conversion) + 2 Recharts AreaCharts (signups + conversions) + HideTestToggle
src/app/(admin)/admin/users/page.tsx           Users table: search, filter pills, HideTestToggle, paginated, expandable rows with copyable IDs (shared InfoField), delete dialog (sole-admin radio: user only vs user+household)
src/app/(admin)/admin/households/page.tsx      Households table: search by name or invite code, filter, HideTestToggle, Set Premium/Free button, ConfirmDialog, optimistic update, expandable rows (shared InfoField)
...
src/app/api/admin/promo-codes/route.ts        GET (list all) + POST (create with auto-gen or custom code; duplicate custom code returns 409 DUPLICATE_CODE instead of an unhandled 500)
...
src/app/api/household/[id]/route.ts            PATCH: rename household (admin); DELETE: hard delete all content + household (uses shared deleteAllHouseholdData)
src/lib/utils/deleteHouseholdData.ts           deleteAllHouseholdData(householdId): shared FK-ordered wipe of all household content (reminders, chores, grocery, calendar, notes, tasks, expenses, meals, activity); leaves household row + members + config rows. Used by [id] DELETE and [id]/delete-data POST.
...
src/app/api/invite/[token]/route.ts           GET (public): check invite validity, return household_name + expires_at + email (404 not_found / 410 expired-or-used / 200 valid); POST (auth): join as guest role with the reduced guest permission set (expensesView/Add, groceryAdd, calendarAdd, mealsSuggest = true; everything else off), 409 ALREADY_MEMBER if already a member, mark accepted, set onboardingCompleted, log member_joined
src/app/api/cron/guest-expiry/route.ts        Daily 2am UTC: hard-delete expired guest member_permissions rows, soft-delete the household_members row (deleted_at=now, matching the app-wide soft-delete convention; the isNull(deleted_at) filter keeps re-runs idempotent), logs guest_expired activity. Expired guests are already blocked everywhere via isNull(deleted_at) filters and getUserHousehold returning null.
...
src/app/api/household/me/route.ts             GET: current user's household + role + permissions; resolves the household via getUserHousehold (honours users.active_household_id, falls back to most-recent) so useHousehold() reflects the active household after switching
...
- Seed accounts (all email/password accounts use password RoostTest123!). See docs/TEST-ACCOUNTS.md for the full QA reference.
  - ROOST PREMIUM HOUSE (invite code PREMHS, premium) — main QA playground:
    admin.premium@roost.test (admin), jordan@roost.test (Jordan Lee, member), taylor@roost.test (Taylor Kim, member),
    riley.guest@roost.test (Riley Guest, guest role, expires in 14 days), Premium Kid (child, PIN 5678, code PREMHS).
  - ROOST FREE HOUSE (invite code FREEHS, free) — free-tier limit testing:
    admin.free@roost.test (admin), member@roost.test (Test Member, member), Test Child (child, PIN 1234, code FREEHS).
  - ROOST SECOND HOUSE (invite code SECND2, premium) — exists so admin.premium@roost.test belongs to two households (multi-household switcher QA). Minimal content.
  - Promo codes seeded for redemption QA: ROOSTFREE30 (active 30d), ROOSTLIFE (active lifetime), PAUSEDCODE (paused), DEADCODE (deactivated), MAXEDOUT (active, maxRedemptions reached), EXPIREDPROMO (active but expired).
- src/db/seed.ts: idempotent QA seeder, run via `npm run db:seed`. Creates the accounts above (better-auth user + credential account providerId 'credential' + app users row + household + members + permissions; children are PIN-only with placeholder email and no credential row, hashed via hashPassword from better-auth/crypto), plus rich cross-feature content so the GitHub QA checklists (#24-#43, #62) can be exercised by hand. Premium House is seeded with chores assigned to specific members + completions (leaderboard/streaks/history), expenses with unsettled + pending-claim splits creating real who-owes-who balances, a budget, a savings goal with contributions, recurring + bill templates, upcoming/recurring calendar events with an RSVP event, a default + a second grocery list with checked/unchecked items, tasks incl. a project/subtask/comment/delegation, plain + rich-text notes, one-time/recurring/due-now reminders (the due-now one drives the ReminderBanner), a meal bank + planner slots (tonight's dinner) + a suggestion with votes, a reward rule + an unacknowledged payout for the child, and ~12 activity rows. Free House gets a lighter set deliberately kept under free-tier limits. Each content block only seeds when that table is empty for the household, so re-running never duplicates. tsx --env-file=.env.local loads env vars.
...
- POST /api/household/invite expects camelCase { expiresInDays } and returns { token, url, expiresAt, linkExpiresAt } (token at top level, not nested under invite).
- POST /api/household/members/add-child returns 200 (not 201) with { child: { id, name }, pin }; it validates only non-empty name + 4-digit PIN (no server-side name length cap). The free tier caps children at 1 (seed already makes one child per house), so child-creation tests that reuse a seeded free session must use the premium-admin session instead (child-upgrade.spec does this).
- Cron success shapes: reminders { processed }, rewards { payoutsCreated, expensesCreated }, subscription { expired }, settlement-reminders { notified }, recurring-expenses { created, skipped }, budget-reset { reset }, guest-expiry { removed }. All also include ok:true where noted.
- Chores premium gate copy is "Unlock recurring chores" + button "Upgrade for $4/month". The Leaderboard header button is a premium lock: it carries aria-disabled=true for free users, so Playwright needs click({ force: true }); its React onClick still opens the gate sheet (role=dialog). The History header button navigates to /chores/history, which renders a page-level PremiumGate for free users (no dialog role). The stats free gate copy is "Unlock household stats".
- Quarantined (test.skip) as V2-removed behavior: permissions.spec "POST /api/expenses -> 403" (expense creation is free now) and household.spec "add-child name longer than 32 -> 400" (no server-side length validation).

Last updated: 2026-07-07 (E2E restore + V2 drift triage, issue #99. Merged main (incl. #104 chore-category removal) into fix/e2e-restore-hooks, then drove the Playwright suite green: 141 passed, 2 skipped, 0 failed locally. Fixes were all test-side (no app behavior changed): onboarding "Go to Roost" CTA in helpers/auth.ts + onboarding.spec; /expenses page -> /money in permissions.spec with Bills-tab premium gate assertions; household.spec rewritten for the current create/join/remove/rename response shapes, join-approval pending flow, already-member 409, add-child 200 + child cap (moved child-upgrade.spec to premium-admin); requireSession() redirect-to-login auth-gate assertions; dropped the nonexistent /api/household/activity route; cron.spec response-shape fixes for rewards/settlement-reminders/recurring-expenses/guest-expiry; auth-invite.spec camelCase expiresInDays + top-level token; chores.spec rewritten to deterministic Leaderboard + History gates with current "Unlock recurring chores" copy; stats gate copy "Unlock household stats". Two stale specs quarantined with comments. Verified hooks present: login-submit, signup-submit, chore-save-btn, plus grocery-quick-add; FREE_HOUSEHOLD_CODE/PREMIUM_HOUSEHOLD_CODE = FREEHS/PREMHS. Note: the app has no dashboard-tiles hook in V2 — the dashboard tile grid was replaced by /today + sidebar/bottom nav, so premium.spec checks core routes load instead.)

Previous: 2026-07-06 (Admin panel QA pass, issue #38, plus fixes #94/#95.
1. Delete-user FK bug (#95): admin "Delete user only" hard-deleted the app users row, which FK-violates because ~20 content tables (household_activity, chores, expenses, tasks, grocery, meals, reminders, rewards, notes, calendar, notifications, promo_redemptions, ...) reference users.id with no onDelete rule. Rewrote src/app/api/admin/users/[id]/route.ts to ANONYMIZE instead: revoke sessions/accounts, drop memberships + member_permissions, scrub PII and set users.deleted_at (row kept so content FKs stay valid), tombstone the better-auth user email. Surviving households keep history as "Former Member" (matches the documented Former-Member design). The "Delete user + household" path now uses a single DELETE FROM households (every households.id FK is onDelete cascade), replacing the redundant/incomplete manual per-table SQL.
2. Hide-test-accounts filter (#94): the feature existed in V1 but was dropped when apps/web (V2) was promoted to root (commit 412a19c). Recreated src/lib/admin/testFilters.ts as the single source of truth (isTestUserSql/isTestHouseholdSql + EXCLUDE_TEST_* fragments), fixing a latent bug where email ILIKE '%test%' returned NULL for PIN-only children and hid them (now COALESCE(email,'')). Added src/lib/admin/useHideTest.ts (useSyncExternalStore over localStorage, hydration-safe) + src/components/admin/HideTestToggle.tsx, wired ?hideTest into the stats/users/households routes and a toggle onto all three admin pages (consistent + persistent).
3. Closing out #38: copyable IDs are now real (shared src/components/admin/InfoField.tsx with a copy button on mono/ID fields, replacing the per-page InfoField); duplicate custom promo code now returns 409 DUPLICATE_CODE instead of an unhandled 500 (src/app/api/admin/promo-codes/route.ts). Also corrected stale CLAUDE.md drift: the (admin)/layout.tsx is a passthrough (auth is via the API routes + client 401 redirect, not a server x-pathname gate), and the overview has 4 StatCards not 6. No schema change, no db:push needed. Verified with tsc + eslint + a runtime smoke test (login renders, admin APIs 401 unauth, overview renders).)

Previous: 2026-07-02 (Household management QA pass, issue #35. Audited the household surface and fixed/restored several things.
1. Multi-household switch leak: /api/household/me and /api/household/members were resolving the household by most-recently-joined and IGNORING users.active_household_id, so after using the household switcher the Household page + everything driven by useHousehold() (isPremium/role/permissions/TopBar name) stayed on the wrong household. Both now resolve via getUserHousehold() (active-household-first, most-recent fallback), matching /api/households and the feature routes.
2. Restored three missing household routes whose Settings buttons were 404ing: POST /api/household/[id]/regenerate-code (new unique 6-char code, returns {code}); POST /api/household/[id]/transfer-admin (body {newAdminUserId}; validates target is an active non-child non-self member; promotes target first then demotes caller so the household is never admin-less; logs admin_transferred); POST /api/household/[id]/delete-data (wipes content, keeps household + members). Extracted the FK-ordered content wipe into src/lib/utils/deleteHouseholdData.ts, now shared by [id] DELETE and [id]/delete-data.
3. Restored the guest-invite flow (it had been stubbed to "coming soon"): src/lib/utils/inviteToken.ts (generateInviteToken 32-byte hex + getInviteUrl); POST /api/household/invite (admin + requirePremium; body {email?, expiresInDays 1-365}; membership expiresAt = now+N days, linkExpiresAt = min(now+7d, expiresAt)); GET/POST /api/invite/[token] (public validity 404/410/200; auth accept creates guest membership with the reduced guest perm set, 409 ALREADY_MEMBER, marks accepted, sets onboardingCompleted, logs member_joined); public landing page src/app/invite/[token]/page.tsx (loading/valid/not_found/expired/used/error; signed-in Join-as-guest, signed-out signup/login carrying ?callbackUrl=/invite/[token]); rebuilt src/components/settings/InviteGuestSheet.tsx (email optional, expiry preset pills, generate link, copy/share). Signup page now honours ?callbackUrl= (login already did). Guest-expiry cron + household_invites schema + GUEST_MEMBER_PREMIUM gate already existed; no db:push needed.
4. Household page member row now shows an amber "Expires in N days" caption for guest members (member.expiresAt was already returned by the API but unused).
Known follow-ups left as separate issues: guest DEFAULT_PERMISSIONS in members/route.ts are broader than the accepted-guest set (cosmetic, since accepted guests get an explicit perms row); checkMultiHouseholdLimit in join/route.ts picks an arbitrary membership (no orderBy) to gate premium; there is still no general member<->child role picker (only transfer-admin changes roles); desktop Sidebar shows the household switcher chrome even for single-household free users (mobile TopBar correctly hides it).)

Previous: 2026-07-02 (Stats QA pass, issue #34. Verified free full-page gate + server 403 (STATS_PREMIUM), premium access, all date-range pills + custom range, stat cards, all 6 charts, zero-data empty states, and mobile layout against seeded accounts. Three cleanups: (1) stats page now uses the shared PremiumGate (feature="stats" trigger="page") instead of a local InlinePremiumGate, for gate consistency. (2) Removed the dead overdueCount task query + response field from /api/stats (computed but never rendered); dropped now-unused lt import. tasks response shape is now {totalCreated, totalCompleted, completionRate, byPriority}. (3) Silenced Recharts "width(-1)/height(-1)" console warnings (recharts 3.8.1 hardcodes isDev=true so they fire in production too) by passing initialDimension={CHART_INIT} to all 6 ResponsiveContainers.)
...
4. Money /money page title: fontSize 24 -> 26, letterSpacing -0.5px -> -0.3px to match all other pages.
5. SectionGroup shared component: created src/components/shared/SectionGroup.tsx with variant="card" (slab wrapper, chores pattern) and variant="list" (header above individual SlabCards, tasks/reminders pattern). Removed local SectionCard from chores/page.tsx and local SectionHeader from tasks/page.tsx and reminders/page.tsx. All three pages now import SectionGroup.
6. Subtitle format standardized: notes changed "X notes" -> "X active"; chores inline count badge removed, subtitle "X active" added below h1. All feature pages now use "X active" subtitle format.
Content max-width pass (earlier in session, committed separately): PageContainer changed from max-w-4xl to max-w-3xl (768px). Most pages standardized to 768px. Meals and Money use 900px outer wrapper for wider content. Calendar month grid unchanged at 1024px. Loading skeletons now match content max-width on reminders, today, and lists pages. Grocery page background changed from #fff to #F9FAFB throughout. Money page header and tab strip backgrounds changed from var(--roost-surface) to #F9FAFB. Meals planner: removed desktop empty state, removed FAB, added inline "+ Add Meal To Planner" button in week nav row. Meal Bank and Suggestions mobile + header buttons removed.)

Previous: 2026-04-15 (Promo code system upgraded: lifetime promo codes + per-account redemption block. Schema: is_lifetime boolean added to promo_codes (default false); premium_expires_at made nullable on promo_redemptions (null = lifetime); db:push applied. Redemption API: lifetime codes set premium_expires_at=null for permanent premium; duplicate check now blocks all past redemptions per code+household (not just active). Admin API POST accepts isLifetime, skips duration validation when true. requirePremium() comment clarified: null premium_expires_at = permanent premium (no lazy revert). Admin UI: lifetime toggle (purple switch + Infinity icon) in create form disables Duration dropdown; table shows purple "Lifetime" badge. Promo status API returns lifetime redemptions (isNull OR future expiry). Settings promo cards show "Never expires" with Infinity icon for lifetime.)
