...
> Last updated: 2026-04-15 (Promo code system upgraded: lifetime promo codes + per-account redemption block. New is_lifetime boolean on promo_codes; premium_expires_at nullable on promo_redemptions. Lifetime codes set premium_expires_at=null for permanent premium. Duplicate check blocks all past redemptions per code+household, not just active ones. Admin UI gains lifetime toggle + purple badge. Settings shows "Never expires" with Infinity icon for lifetime redemptions.)
...
- Promo Codes page: create codes (auto-generated or custom), set duration (30/60/90/180/365 days) or lifetime, optional max redemptions, optional code expiry date. Lifetime toggle (purple, Infinity icon) disables duration picker. Filter tabs (All/Active/Paused/Deactivated). Status actions: Pause, Activate, Deactivate, Reactivate. Copy code to clipboard. Monospace code display, strikethrough for deactivated codes. Lifetime codes show purple "Lifetime" badge in Duration column.
- Spanish localization

#### Promo Code System
Admin-created promo codes that grant time-limited or lifetime premium access to households.
- Admin creates codes in /admin/promo-codes with duration (or lifetime toggle), max redemptions, and optional expiry
- Lifetime codes: is_lifetime=true on promo_codes, sets premium_expires_at=null (never expires)
- Time-limited codes: duration_days sets premium_expires_at = now + duration
- Users redeem codes in Settings > Promotions section
- Code input auto-uppercases, validates server-side (status, expiry, max redemptions, per-account duplicate block)
- Per-account duplicate block: checks all past redemptions for the same code + household, not just active ones. Even expired redemptions block re-use.
- Redemption sets household subscription_status='premium' + premium_expires_at (null for lifetime, date for time-limited)
...
- Active promo status shown in Settings with code name, Active badge, and expiry date (or "Never expires" with Infinity icon for lifetime)
- requirePremium() server-side: premium_expires_at=null means permanent premium (Stripe or lifetime promo), lazy cleanup only fires when premium_expires_at is non-null and in the past
