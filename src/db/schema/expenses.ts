...
export const expenses = pgTable(
  ...
  (t) => ({
    householdCreatedIdx: index("expenses_household_created_idx").on(
      t.household_id,
      t.created_at
    ),
    householdPaidByIdx: index("expenses_household_paid_by_idx").on(
      t.household_id,
      t.paid_by
    ),
  })
...
export const expense_splits = pgTable(
  ...
  (t) => ({
    expenseUserIdx: index("expense_splits_expense_user_idx").on(
      t.expense_id,
      t.user_id
    ),
    userSettledIdx: index("expense_splits_user_settled_idx").on(
      t.user_id,
      t.settled
    ),
  })
