...
export async function cleanupPlaywrightTestData(): Promise<void> {
  ...
  if (householdIds.length > 0) {
    ...
    await db.delete(promo_redemptions).where(inArray(promo_redemptions.household_id, householdIds));
    await db.delete(household_invites).where(inArray(household_invites.household_id, householdIds));
    await db
      .delete(household_join_requests)
      .where(inArray(household_join_requests.household_id, householdIds));
    await db.delete(reminders).where(inArray(reminders.household_id, householdIds));
    await db.delete(chore_streaks).where(inArray(chore_streaks.household_id, householdIds));
    await db.delete(chores).where(inArray(chores.household_id, householdIds));
    await db.delete(chore_categories).where(inArray(chore_categories.household_id, householdIds));
    await db.delete(grocery_items).where(inArray(grocery_items.household_id, householdIds));
    await db.delete(grocery_lists).where(inArray(grocery_lists.household_id, householdIds));
    await db.delete(calendar_events).where(inArray(calendar_events.household_id, householdIds));
    await db.delete(notes).where(inArray(notes.household_id, householdIds));
    await db.delete(tasks).where(inArray(tasks.household_id, householdIds));
    await db
      .delete(recurring_expense_templates)
      .where(inArray(recurring_expense_templates.household_id, householdIds));
    await db.delete(expenses).where(inArray(expenses.household_id, householdIds));
    await db.delete(expense_budgets).where(inArray(expense_budgets.household_id, householdIds));
    await db.delete(expense_categories).where(inArray(expense_categories.household_id, householdIds));
    await db.delete(meal_suggestions).where(inArray(meal_suggestions.household_id, householdIds));
    await db.delete(meal_plan_slots).where(inArray(meal_plan_slots.household_id, householdIds));
    await db.delete(meals).where(inArray(meals.household_id, householdIds));
    await db.delete(household_activity).where(inArray(household_activity.household_id, householdIds));
    await db.delete(reward_payouts).where(inArray(reward_payouts.household_id, householdIds));
    await db.delete(reward_rules).where(inArray(reward_rules.household_id, householdIds));
    await db.delete(allowance_payouts).where(inArray(allowance_payouts.household_id, householdIds));
  ...
  if (allUserIds.length > 0) {
    await db.delete(member_permissions).where(inArray(member_permissions.user_id, allUserIds));
    await db.delete(household_members).where(inArray(household_members.user_id, allUserIds));
    await db.delete(notification_queue).where(inArray(notification_queue.user_id, allUserIds));
    await db
      .delete(household_join_requests)
