    ...
    <>
      <motion.div
        ...
        style={{
          backgroundColor: "var(--roost-surface)",
          border: "1.5px solid var(--roost-border)",
          borderBottom: overdue ? "4px solid #EF444460" : `4px solid ${COLOR}30`,
          minHeight: 64,
      ...
      >
        ...
        >
          ...
          >
            {reminder.completed && <Check className="size-3.5 text-white" strokeWidth={3} />}
          </div>
        </button>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            <SectionColorBadge label={freqLabel(reminder.frequency)} color={COLOR} />
            {isRecurring && <RefreshCw className="size-3" style={{ color: COLOR }} />}
            <span
              className="flex items-center gap-1 text-xs"
              style={{ color: overdue ? "#EF4444" : "var(--roost-text-muted)", fontWeight: 600 }}
            >
        ...
        <DialogContent>
          ...
          <DialogFooter className="mt-2 gap-2">
            ...
            <motion.button
              type="button"
              whileTap={{ y: 1 }}
              onClick={() => { setConfirmOpen(false); onComplete(); }}
...
export default function RemindersPage() {
  ...
  const completeMutation = useMutation({
    ...
    onError: (_err, _id, ctx) => {
      ...
      toast.error("Could not complete reminder", { description: "Something went wrong. Try again." });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: Error) => toast.error(err.message, { description: "Could not delete the reminder. Try again." }),
  });
  ...
  return (
    ...
    >
      ...
      <ReminderSheet
        open={sheetOpen}
        onClose={() => { setSheetOpen(false); setSelectedReminder(null); }}
        mode={sheetMode}
