...
export default function MealsPage() {
  ...
  const voteMutation = useMutation({
    ...
    onMutate: async ({ id, vote }) => {
      ...
      const prev = queryClient.getQueryData<SuggestionsResponse>([
        'suggestions',
      ]);
      queryClient.setQueryData<SuggestionsResponse>(['suggestions'], (old) => {
        ...
        return {
          suggestions: old.suggestions.map((s) => {
            ...
            const upvotes =
              s.upvotes +
              (vote === 'up' ? (wasVoted ? -1 : 1) : 0) +
              (s.userVote === 'up' && vote === 'down' ? -1 : 0);
            const downvotes =
              s.downvotes +
              (vote === 'down' ? (wasVoted ? -1 : 1) : 0) +
              (s.userVote === 'down' && vote === 'up' ? -1 : 0);
            return {
              ...s,
              upvotes,
              downvotes,
  ...
  function openSlotForMeal(meal: MealData) {
    setSlotSheet({
      ...
      slotType: 'dinner',
      existingSlot: null,
      preSelectedMeal: meal as MealRow,
    });
  }
                ...
                >
                  ...
                  <div style={{ padding: '8px 14px 10px' }}>
                    {SLOT_TYPES.map((slotType) => {
                      ...
                      const mealName =
                        slot?.meal_name ?? slot?.custom_meal_name;
                      return (
                        <button
                          key={slotType}
                          type="button"
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <motion.button
                  ...
                  whileTap={{ y: 1 }}
                  onClick={() =>
                    voteMutation.mutate({ id: s.id, vote: 'down' })
                  }
                  className="flex h-9 items-center gap-1.5 rounded-xl px-3"
                  style={{
                    backgroundColor: userDownvoted
                      ? '#EF444418'
                      : 'var(--roost-bg)',
