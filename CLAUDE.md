...
- Seed accounts: admin.free@roost.test (free admin, Roost Free House, invite code FREEHS), admin.premium@roost.test (premium admin, Roost Premium House, invite code PREMHS), member@roost.test (member in free house), Test Child (child in free house, PIN 1234). All passwords: RoostTest123!
- src/db/seed.ts: idempotent seed script, run via `npm run db:seed`. Creates better-auth user + credential account (providerId 'credential') + app users row + household + members + permissions. Child is PIN-only (placeholder email, no credential row). Uses hashPassword from better-auth/crypto for correct scrypt format. tsx --env-file=.env.local loads env vars. Safe to re-run (every step checks for an existing row first).
